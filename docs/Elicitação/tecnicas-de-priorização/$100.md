...
| ID | Descrição | Tipo | Preço |
| -- | --------- | ---- | ---- |
| OBS05 | O usuário deve ser notificado sobre prazos e documentos vencidos. | RF |  |
| OBS01 | O usuário deve conseguir consultar multas e débitos do veículo. | RF |  |
| BS03 | O usuário deve conseguir pagar taxas e débitos diretamente pelo aplicativo. | RF |  |
| BS06 | O usuário deve ser notificado pelo aplicativo em caso de multas, com informações sobre prazos de pagamento e descontos disponíveis. | RF |  |
| IS04 | O usuário deve conseguir realizar a transferência de autuação de infração para outro condutor. | RF |  |
| BS01 | O usuário deve conseguir visualizar a pontuação da sua Carteira Nacional de Habilitação (CNH).  | RF |  |
| BS04 | O usuário deve ser notificado pelo aplicativo em caso de furto do veículo e também em caso de recuperação. | RF |  |
| BS08 |Opção para adicionar quilometragem do seu carro  | RF |  |
| BS09 | O usuário gostaria de ter um guia com os documentos necessários para cada tipo de serviço. | RF |  |
| IS06 | O usuário deve conseguir realizar a transferência de propriedade do veículo. | RF |  |
| IS10 | O usuário gostaria que o aplicativo fosse compatível com diferentes modelos e sistemas operacionais de dispositivos móveis. | RNF |  |
| BS18 | O usuário gostaria de ter um leitor de tela e assistente por voz para pessoas cegas. | RNF |  |
| IS09 | O usuário gostaria que fosse possível acessar diferentes perfis de usuário em um dispositivo | RNF |  |
| IS11 | O usuário gostaria de ter segurança na exibição de dados pessoais | RNF |  |
| IS02 | O usuário deve conseguir agendar serviços como vistoria, emissão de documentos, entre outros. | RF |  |
| IS07 | O usuário espera que a interface do aplicativo seja intuitiva e fácil de usar.  | RNF |  |
| BS11 | O usuário gostaria que o aplicativo tivesse integração com os apps CNH Digital e Gov.br.  | RF |  |
| BS07 | O usuário deve conseguir agendar provas teóricas e práticas pelo aplicativo | RF |  |
| BS12 | O usuário gostaria de ter atendimento online ao vivo com um servidor do Detran, via chat ou vídeo.  | RF |  |
| BS16 | O usuário gostaria de uma área educacional dentro do aplicativo sobre processos do DETRAN (explicações, vídeos, etc.)| RNF |  |
